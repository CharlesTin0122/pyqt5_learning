...
class Widget(QWidget):  # QWidget是PyQt中所有窗口类的基类，所有的控件和窗口都是通过其拓展出来的，实例化后生成空白窗体。
	def __init__(self):  # 构建函数
		...
		self.setStyleSheet('background:rgb(100,200,200)')  # 通过样式表的方法设置窗口颜色
		self.setWindowFlags(Qt.WindowStaysOnTopHint)  # 设置窗口旗标，置顶旗标
...
if __name__ == '__main__':  # 如果是本程序运行
	...
	sys.exit(app.exec_())  # app.exec_()使app实例运行起来，sys.exit用来捕获程序的异常退出
